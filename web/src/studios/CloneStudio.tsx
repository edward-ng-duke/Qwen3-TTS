...
import { CloneComposer } from "@/studios/base/CloneComposer"
...
export function CloneStudio() {
  ...
  const loadFromHistory = useCloneStore((s) => s.loadFromHistory)

  const studioApi = useMemo<StudioApi>(() => ({
    reuse: (item) => loadFromHistory({ text: item.text, language: item.language, seed: item.seed }),
  }), [loadFromHistory])
  ...
  return (
    <AuthGate>
      <VariantThemeProvider variant="base">
              ...
              <main className="flex-1 min-w-0 overflow-y-auto overscroll-contain">
                <div className="w-full max-w-3xl mx-auto px-3 sm:px-4 md:px-6 pt-[calc(5.75rem+var(--safe-top))] pb-[calc(6.75rem+var(--safe-bottom))] md:pb-10 space-y-5 sm:space-y-6">
                  <section className="max-w-[880px] mx-auto pt-1 sm:pt-3">
                    <p className="text-[12px] font-medium text-[var(--brand)]">{T.clone.eyebrow}</p>
                  ...
                  <div>
                    <div className="max-w-[880px] mx-auto mb-3 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                      <div>
                        <h2 className="text-base font-semibold text-[var(--text-primary)]">{T.clone.resultsTitle}</h2>
                        <p className="text-[12px] leading-relaxed text-[var(--text-tertiary)]">
                          {T.clone.resultsDescription}
                        </p>
                      </div>
                    </div>
                    <ResultsStream />
